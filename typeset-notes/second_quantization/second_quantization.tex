...
\newtheorem{pos}[mot]{Postulate}
\newtheorem{res}[mot]{Result}
\newtheorem{rem}[mot]{Remark}
\newtheorem{dfn}[mot]{Definition}
\newtheorem{eg}[mot]{Example}
\newtheorem{recipe}[mot]{Recipe}
\newtheorem{thm}[mot]{Theorem}
\newtheorem{lem}[mot]{Lemma}
...
\begin{abstract}
    ...
    Being familiar with the trick of re-labelling
    summation index is important for understanding
    calculations in the first section.
...
\begin{dfn}[Permutation, exchange, parity]
    ...
    Exchanges are often denoted as
     \[P: i \leftrightarrow j \coloneqq P: i \mapsto j, j \mapsto i. \]
...
\begin{lem}
It follows for Fermions that if $ \exists i,j \text{ s.t. } \alpha_{i}=\alpha_{j}$ then
$\mathcal{P} |\alpha_{1},\cdots,\alpha_{N})=\ket{\text{null}}.$
We can consider two copies of $\mathcal{P} |\alpha_{1},\cdots,\alpha_{N})$,
and in the first copy,
for every permutation $P$ we pair it with
the term with
$P \circ (i \leftrightarrow j)$
in the other copy.
This pair of terms sums up to $\ket{\text{null}}$,
and thus $2 \mathcal{P} |\alpha_{1},\cdots,\alpha_{N}) = 0$,
our claim is proven.
This is commonly known as the Pauli exclusion principle.
\end{lem}
...
\begin{rem}
    ...
    Let us focus on the Bosonic or Fermionic states ``constructed'' by $\mathcal{P}$ first,
    since they can be written down explicitly.
    Denoting $\mathcal{P} |\alpha_1, \ldots ,\alpha_{N})$ with 
    $\ket{\alpha_1, \ldots ,\alpha_{N}}$, 
    we only need to check that after applying permutation $P$
    to this state, the result
    \[
    ...
    \]
    equals to 
    $\zeta^{\abs{P}} \mathcal{P} |\alpha_1, \ldots ,\alpha_{N})$.
    ...
    Now we turn to a generic $+1$ eigenstate $\ket{\psi}$ of $\mathcal{P}$
    and prove that it has the commonly known property.
    We want to write $\ket{\psi}$
    in a form similar to
    $\mathcal{P} | \alpha_{1},\cdots, \alpha_{N})$
    since we have just proved our claim for
    those explicitly constructed eigenstates.
    Naively expanding $\ket{\psi}$ under the product basis
    is not enough, for this only gives product states,
    but we still need a $\mathcal{P}$ in front of them.
    We can prepend it manually,
    because:
    \[\ket{\psi} = \mathcal{P} \ket{\psi}\]
    and, upon
    expanding $\mathcal{P} \ket{\psi}$ in the product state basis, we have
    \[
    \mathcal{P} \ket{\psi} = 
    \sum_{\alpha_{1},\cdots, \alpha_{N}}
    C_{\alpha_{1},\cdots, \alpha_{N}}
    \mathcal{P} | \alpha_{1},\cdots,\alpha_{N}),
    \]
    which is a superposition of components
    $\mathcal{P} | \alpha_{1},\cdots,\alpha_{N})$.
    The commonly known property is linear;
    if it holds for all components then it
    also holds for any superposition:
    a permutation produces an overall phase $\zeta^{\abs{P}}$
    for each term, and all terms have the same phase,
    so this phase can be factored out of the summation.
    Our proof for generic eigenstates is therefore complete.
...
\end{mot}
    ...
    the only contributing permutation
    in the Fermion case is the identity, due to
...
This note features conceptual clarity because
it shows second quantization to be simply a basis
...
The seemingly strange projector $\mathcal{P}$ is just
a rigorous way to specify what we mean by
a (anti)symmetrized state,
and its behavior is easily understood with the
re-labelling trick.
